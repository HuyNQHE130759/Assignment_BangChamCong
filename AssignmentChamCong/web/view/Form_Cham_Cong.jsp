...
<%@page import="model.WorkingTable"%>
<%@page import="dal1.DAO"%>
<%@page import="java.util.List"%>
<%@page import="model.WorkingbyEmployeeList"%>
<%@page import="model.workingDateList"%>
...
<html>
    ...
    <table border="1" width="95%" height="150px" style="margin-left:auto; margin-right:auto;">
        ...
        <!--            // tung nguoi 1-->
        <%ArrayList<Employee> el = (ArrayList<Employee>) request.getAttribute("employeeList");%>
         <%ArrayList<workingDateList> wdl = (ArrayList<workingDateList>) request.getAttribute("workingDateList");%>
          <%ArrayList<WorkingbyEmployeeList> wdbel = (ArrayList<WorkingbyEmployeeList>) request.getAttribute("WorkingbyEmployeeList");%>
        ...
        <tr>
            <td><%=(i+1)%></td>
            <td><%=el.get(i).geteName()%></td>
            <% for (int j = 0; j < 31; j++) {
                    wd = null;
                    wd = da.getWorkingDateListDatabyEmployeeByMonth(8, (j + 1), el.get(i).geteID());
                    //System.out.println(wd);
                    if (wd == null ){%>
                        <td><a href="#">Edit</a></td>
                        <% }else{ 
                            for (int k = 0; k < wd.getWdl().size(); k++) {
                                wt = da.getWorkingTableDetailVeryGood(wd.getWdl().get(k).getWorkingDateID());%>
                                 <td><%=wt.getTimeKeepName()%>: <%=wt.getTotalTime()%><br> OT: <%=wt.getOtTime()%> <a href="#">Edit</a></td>
                        <% totalTime += wt.getTotalTime();  }
                        %>
